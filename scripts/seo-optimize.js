...
// The 44 older legacy-template blog posts (scraped WordPress pages, not
// generated by scripts/generate-blog-posts.js) don't set their own hreflang
// tags, so they fall through to this script's generic per-page pass below.
// Every post in blog-posts-en.json has a real English translation now —
// deriving the mapping from that file (rather than hardcoding 44 slugs here)
// keeps this correct automatically if more posts are ever translated.
...
/**
 * Both sides of a page pair, whichever side we were handed.
 *
 * This used to assume every page it saw was German: it labelled `pagePath`
 * hreflang="de" and looked it up in DE_TO_EN. That is true of almost every
 * page it reaches, because the English pages generated by
 * generate-en-pages.js write their own hreflang including x-default and are
 * skipped by the "already has canonical + x-default" test below.
 *
 * An English page that does *not* write x-default therefore fell through and
 * was labelled as German pointing at itself — telling Google the English
 * accessibility statement is the German version of the English accessibility
 * statement. Deriving the language from the path fixes the whole class
 * rather than the one page that exposed it.
 *
 * x-default points at the German page either way: virtual-marketer.de is a
 * German company selling primarily into a German-speaking market, and the
 * German page is the one an unmatched visitor should land on.
 */
const EN_TO_DE_PATHS = Object.fromEntries(Object.entries(DE_TO_EN).map(([de, en]) => [en, de]));

function hreflangBlock(pagePath) {
  const isEnglish = pagePath.startsWith('/en/');
  const dePath = isEnglish ? EN_TO_DE_PATHS[pagePath] || null : pagePath;
  const enPath = isEnglish ? pagePath : DE_TO_EN[pagePath] || null;

  const abs = (p) => `https://virtual-marketer.de${p}`;
  let out = '';
  if (dePath) out += `  <link rel="alternate" hreflang="de" href="${abs(dePath)}">\n`;
  if (enPath) out += `  <link rel="alternate" hreflang="en" href="${abs(enPath)}">\n`;
  out += `  <link rel="alternate" hreflang="x-default" href="${abs(dePath || enPath || pagePath)}">\n`;
  return out;
}
